...
abstract class Curl
{
    ...
    public function setHeaders(array $headers)
    {
        ...
        foreach ($headers as $key => $value) {
            if (is_numeric($key)) {
                ...
                $newHeaders[] = sprintf("%s:%s", $key, $value);
            }
        }
        $this->a_header = $newHeaders;
    }
    ...
    protected function sendRequest()
    {
        ...
        try {
            $error = curl_error($this->r_curl);
            if (!empty($error)) {
                throw new \Exception($error);
            } else {
                $code = curl_getinfo($this->r_curl, CURLINFO_HTTP_CODE);
    ...
    public function setOpts(array $extraOpts = [])
    {
        $a_opts = [
            ...
            CURLOPT_HTTPHEADER         => $this->a_header
        ];
        //合并Options
        $a_opts += $extraOpts;
    ...
    public function sepHeaderBody($s_response)
    {
        ...
        if (strstr($s_header, 'Content-Type: image/jpg') !== FALSE):
            ...
            $m_attachmentName = uniqid() . '.' . trim($a_match[1], '\"');
        endif;
        //响应体
